...
declare_id!("CrossDex11111111111111111111111111111111");
...
pub mod cross_dex_router {
    ...
    pub fn execute_optimal_route(
        ...
        let routing_fee = calculate_routing_fee(
        ...
        route_state.actual_slippage = calculate_total_slippage(
            optimal_route.expected_output,
            final_output,
        );
...
fn find_optimal_route(
    ...
    // Direct route (1 hop)
    if let Some(direct_hop) = find_direct_route(
        input_token,
        output_token,
        input_amount,
        preferred_dexes,
...
fn find_direct_route(
    ...
    for dex in dexes {
        if let Some(hop) = simulate_dex_swap(&dex, input_token, output_token, input_amount)? {
            if best_hop.is_none() || hop.expected_output > best_hop.as_ref().unwrap().expected_output {
                best_hop = Some(hop);
            }
        }
    }
...
fn find_multi_hop_route(
    ...
    for intermediate in intermediate_tokens {
        ...
        if let Some(hop1) = find_direct_route(input_token, intermediate, input_amount, preferred_dexes)? {
...
fn execute_route_hops(
    ...
    for (i, hop) in route.hops.iter().enumerate() {
        ...
        let swap_result = execute_dex_swap(
            &hop.dex,
            current_amount,
            min_output,
            hop.pool_address,
...
fn calculate_total_price_impact(route: &OptimalRoute) -> u16 {
    ...
    route.hops.iter().map(|hop| hop.price_impact as u32).sum::<u32>() as u16
}
